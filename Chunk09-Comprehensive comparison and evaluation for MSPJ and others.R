...
method.list<-list(deg.int,deg.GA,deg.limma,deg.mt,deg.RF,
            deg.RP,deg.sam,deg.snr,deg.t,deg.wt,deg.mrmr)

names(method.list)<-c("MSPJ","GA","limma",
                "multtest","RF","RankProd","SAM",
                "SNR","T.test","Wilcoxon's test","mRMR")


upset(fromList(method.list),
...
GOlist<-list(deg.int,deg.GA,deg.limma,deg.mt,deg.RF,
             deg.RP,deg.sam,deg.snr,deg.t,deg.wt,deg.mrmr)
...
feedback<-try(pROC::ci.coords(res.roc, x="best", input = "threshold", ret=rets),silent=TRUE)
...
if(error_or%in%feedback==TRUE){
  ...
  feedback1<-try(pROC::ci.coords(res.roc, x="best", input = "threshold", ret=rets),silent=TRUE)
  
  if(error_or%in%feedback1==TRUE){
    
    thresholdx<-0.9
    ciorder<-pROC::ci.coords(res.roc, x=thresholdx, input = "threshold", ret=rets)
